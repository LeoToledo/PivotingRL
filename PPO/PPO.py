...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, has_continuous_action_space, action_std_init):
        ...
        if has_continuous_action_space:
            modules = []
            layers = parameters['agent']['mlp']['architecture']
            for i in range(len(layers)):
                if i == 0:
                    modules.append(nn.Linear(state_dim, layers[i]))
                elif i == (len(layers) - 1):
                    modules.append(nn.Linear(layers[i - 1], layers[i]))
                    modules.append(nn.Linear(layers[i], action_dim))
                else:
                    modules.append(nn.Linear(layers[i - 1], layers[i]))
        ...
        for i in range(len(layers)):
            if i == 0:
                ...
                modules.append(nn.Linear(layers[i - 1], layers[i]))
                modules.append(nn.Linear(layers[i], 1))
                break
            else:
                modules.append(nn.Linear(layers[i - 1], layers[i]))
            modules.append(nn.Tanh())
